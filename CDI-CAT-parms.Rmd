...
ability %>% 
  ...
  geom_point() + xlab("Comprehension Ability") + ylab("Production Ability") +
  facet_wrap(. ~ Language) + 
  geom_smooth(method='lm') +
  theme_bw() + 
...
```{r}
en_comp <- lme4::lmer(comprehension ~ age + sex + ability + (1 | data_id), 
                       data=demo$en_comp)
# cor(demo$en_comp$comprehension, demo$en_comp$ability)
```
